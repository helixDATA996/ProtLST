...
class DiagonalGaussian:
    ...
    def sample(self, noise_scale: float = 1.0) -> Tensor:
        return self.mean + noise_scale * torch.exp(0.5 * self.logvar) * torch.randn_like(self.mean)
...
class ProteinVAE(nn.Module):
    ...
    def decode(self, latent: Tensor, mask: Tensor) -> Tensor:
        if self.decoder is None or self.sequence_head is None:
            raise RuntimeError("this VAE was created without an amino-acid sequence decoder")
