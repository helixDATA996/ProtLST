...
def main():
    ...
    seeds=(a.seeds[0],) if a.smoke else tuple(a.seeds); epochs=1 if a.smoke else 3; model_dim=64 if a.smoke else 512; layers=1 if a.smoke else 2; batch_size=4 if a.smoke else 16; bucket_size=128 if a.smoke else 256; bootstrap=10 if a.smoke else 500
    results=[]
    for arm in a.arms:
      for seed in seeds:
        checkpoint=out/f"{arm}.seed{seed}.pt"
        if not a.evaluate_only:
          run([python,str(root/"scripts/vae_trajectory/train_attribution_experiment.py"),"--manifest",a.manifest,"--shard-index",a.shard_index,"--text-cache",a.train_text_cache,"--text-cache",a.validation_text_cache,"--vae",a.vae,"--arm",arm,"--out",str(checkpoint),"--epochs",str(epochs),"--batch-size",str(batch_size),"--bucket-size",str(bucket_size),"--model-dim",str(model_dim),"--layers",str(layers),"--heads","8","--seed",str(seed),"--device",a.device],env)
